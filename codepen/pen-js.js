...
function rebuildEngine() {
  ...
  const newScale = getPixelScale(isCycling);
  if (newScale !== pixelScale) {
    pixelScale = newScale;
    doResize();
  }
  colorLUT = buildLUT(PALETTES[currentPalette].stops, isCycling, PALETTES[currentPalette].bg);
  bgColor = bgABGR(PALETTES[currentPalette].bg);
  document.body.style.background = PALETTES[currentPalette].bg;
  kernel = buildKernel(brushRadius, getBleedRadius(), pixelScale);
  if (!isCycling) {
    for (let i = 0; i < heat.length; i++) heat[i] = Math.min(1, heat[i]);
  }
  updatePanelTheme();
}
